...
public class LoginTasksQueue {
  ...
  private void connectToServer(Logger logger, ConnectedPlayer player, MinecraftConnection connection) {
    if (connection.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_20_2) < 0) {
      ...
      LimboAPI.PendingConfigSession pendingConfigSession = this.plugin.consumePendingConfigSession(this.player);
      if (connection.getActiveSessionHandler() instanceof LimboSessionHandlerImpl sessionHandler) {
        pendingConfigSession = new LimboAPI.PendingConfigSession(sessionHandler.getSettings(), sessionHandler.getBrand());
      }

      // 1.20.2+ client doesn't send ClientSettings and brand while switching state,
      // so we need to carry forward packets captured during LOGIN completion or limbo handoff.
      if (pendingConfigSession != null) {
        if (pendingConfigSession.settings() != null) {
          this.player.setClientSettings(pendingConfigSession.settings());
        }

        // TODO: also queue non-vanilla plugin messages?
        if (pendingConfigSession.brand() != null) {
          try {
            this.server.getEventManager().fireAndForget(new PlayerClientBrandEvent(this.player, pendingConfigSession.brand()));
            SET_CLIENT_BRAND.invokeExact(this.player, pendingConfigSession.brand());
    ...
    this.server.getEventManager().fire(new PostLoginEvent(this.player)).thenAccept(postLoginEvent -> {
      try {
        RegisteredServer nextServer = this.plugin.getNextServer(this.player);
        if (nextServer != null) {
          this.plugin.removeNextServer(this.player);
          this.player.createConnectionRequest(nextServer).fireAndForget();
          return;
        }
