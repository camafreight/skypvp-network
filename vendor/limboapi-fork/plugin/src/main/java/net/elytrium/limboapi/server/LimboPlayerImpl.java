        ...
        this.sessionHandler.disconnect(() -> {
          if (this.plugin.hasLoginQueue(this.player)) {
            if (this.connection.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_20_2) >= 0) {
              this.plugin.storePendingConfigSession(this.player, this.sessionHandler.getSettings(), this.sessionHandler.getBrand());
  ...
  private void sendToRegisteredServer(RegisteredServer server) {
    if (this.connection.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_20_2) >= 0) {
      ...
      this.sessionHandler.disconnectToConfig(() -> {
        ...
        if (this.sessionHandler.getSettings() != null) {
          this.player.setClientSettings(this.sessionHandler.getSettings());
        }

        if (this.sessionHandler.getBrand() != null) {
          try {
            this.plugin.getServer().getEventManager().fireAndForget(new PlayerClientBrandEvent(this.player, this.sessionHandler.getBrand()));
            SET_CLIENT_BRAND.invokeExact(this.player, this.sessionHandler.getBrand());
        ...
        this.plugin.setActiveSessionHandler(this.connection, StateRegistry.CONFIG, handler);
