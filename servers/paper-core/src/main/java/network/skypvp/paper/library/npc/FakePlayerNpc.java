...
/**
 * Client-only player NPC rendered with PacketEvents (no server-side {@link org.bukkit.entity.Player} entity).
 * <p>
 * Used for lobby interact NPCs, corpses, and other display cases where the NPC must not participate in
 * combat or world simulation. For damageable player-like mobs, use a real {@link org.bukkit.entity.LivingEntity}
 * with LibsDisguises — see {@link RealPlayerNpc}.
 */
public final class FakePlayerNpc {
    ...
    public FakePlayerNpc(Plugin plugin, String name, String textureValue, String textureSignature, Location location, boolean glowing) {
        this(plugin, name, textureValue, textureSignature, location, glowing, null, false);
    }

    public FakePlayerNpc(
        Plugin plugin,
        String name,
        String textureValue,
